...
import click
from maclog.log import get_logger  # still needed for deps
...
BLOCKED_DOMAINS = {'com.apple.bluetooth', 'com.apple.MobileAsset', 'com.apple.mobilecal', 'com.apple.momentsd','com.apple.AppleMediaServices', 'com.apple.coremedia', 'com.apple.GEO', 'com.apple.locationd', 'com.apple.MobileBluetooth.debug', 'com.apple.coreimage', 'com.apple.mobilesafari', 'com.apple.powerlogd'}

PREFS = {}

def print_entry(message: str, filename: str, subsystem: Optional[str] = None, category: Optional[str] = None,
                uni: bool = False, color: bool = False, undefined: bool = False) -> None:
    ...
    if uni:
        seen_key = (domain, user, key)
        if seen_key in PREFS:
            return
        PREFS[seen_key] = True
...
@click.option('--uni', is_flag=True, help='output only unique entries')
@click.option('--undefined', is_flag=True, help='filter only non-existing keys')
def mobile(service_provider: LockdownClient, uni, undefined):
    """ Sniff on connected iOS device """
    for entry in OsTraceService(service_provider).syslog():
        if entry.label is None:
            continue
        print_entry(entry.message, entry.filename, entry.label.subsystem, entry.label.category,
                    uni=uni, undefined=undefined, color=user_requested_colored_output())
